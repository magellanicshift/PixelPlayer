...
class LyricsUtilsTest {
    ...
    @Test
    fun calculateWordTimings_distributesTimeAcrossWords() {
        val words = LyricsUtils.calculateWordTimings(
            lineText = "I love you",
            lineStartMs = 1000,
            lineEndMs = 2000
        )
        ...
        assertEquals(listOf("I", "love", "you"), words.map { it.word })
        // "I" has 1 char, "love" has 4 chars, "you" has 3 chars = 8 total
        // Duration = 1000ms, distributed proportionally
        assertEquals(1000, words[0].time) // "I" starts at 1000
        assertEquals(1125, words[1].time) // "love" starts at 1000 + (1/8)*1000 = 1125
        assertEquals(1625, words[2].time) // "you" starts at 1125 + (4/8)*1000 = 1625
    }

    @Test
    fun calculateWordTimings_handlesSingleWord() {
        val words = LyricsUtils.calculateWordTimings(
            lineText = "Hello",
            lineStartMs = 0,
            lineEndMs = 1000
        ...
        assertEquals(0, words[0].time)
    }

    @Test
    fun calculateWordTimings_handlesEmptyString() {
        val words = LyricsUtils.calculateWordTimings(
            lineText = "",
            lineStartMs = 0,
            lineEndMs = 1000
    ...
    @Test
    fun calculateWordTimings_handlesWhitespaceOnly() {
        val words = LyricsUtils.calculateWordTimings(
            lineText = "   ",
            lineStartMs = 0,
            lineEndMs = 1000
    ...
    @Test
    fun calculateWordTimings_groupsWordsForFastSongs() {
        // 10 words in 500ms = 50ms per word, but minimum is 150ms
        // So we can fit 500/150 = 3 groups max
        // 10 words / 3 groups = ~4 words per group (rounded up)
        val words = LyricsUtils.calculateWordTimings(
            lineText = "one two three four five six seven eight nine ten",
            lineStartMs = 0,
            lineEndMs = 500
        )

        // Should group into 3 groups since 500ms / 150ms = 3 max groups
        assertEquals(3, words.size)
        // First group should have multiple words joined with spaces
        assertTrue(words[0].word.contains(" "))
    }
}
