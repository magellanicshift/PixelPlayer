...
object LyricsUtils {
    ...
    /**
     * Calculates synthetic word timings for a lyrics line when word-level timestamps
     * are not available. Distributes time across words proportionally based on character count.
     * Groups words together if they would display too fast (minimum ~150ms per word group).
     * 
     * @param lineText The text of the lyrics line.
     * @param lineStartMs The start time of the line in milliseconds.
     * @param lineEndMs The end time of the line in milliseconds.
     * @param minWordDurationMs Minimum duration per word/group in milliseconds. Default 150ms.
     * @return A list of SyncedWord with calculated timestamps.
     */
    fun calculateWordTimings(
        lineText: String,
        lineStartMs: Int,
        lineEndMs: Int,
        minWordDurationMs: Int = 100
    ): List<SyncedWord> {
        val words = lineText.split(Regex("\\s+")).filter { it.isNotEmpty() }
        if (words.isEmpty()) return emptyList()
        if (words.size == 1) return listOf(SyncedWord(lineStartMs, words[0]))

        val totalDuration = (lineEndMs - lineStartMs).coerceAtLeast(0)
        
        // Calculate how many word groups we can fit with minimum duration
        val maxGroups = (totalDuration / minWordDurationMs).coerceAtLeast(1)
        
        // If we have more words than we can show, group them
        if (words.size > maxGroups) {
            val wordsPerGroup = (words.size + maxGroups - 1) / maxGroups // Ceiling division
            val groupedWords = words.chunked(wordsPerGroup).map { group -> group.joinToString(" ") }
            
            val totalChars = groupedWords.sumOf { it.length }.coerceAtLeast(1)
            var currentTime = lineStartMs
            
            return groupedWords.map { groupText ->
                val groupDuration = (totalDuration * groupText.length) / totalChars
                val syncedWord = SyncedWord(currentTime, groupText)
                currentTime += groupDuration
                syncedWord
            }
        }
        
        // Normal case: distribute time proportionally by character count
        val totalChars = words.sumOf { it.length }.coerceAtLeast(1)
        
        var currentTime = lineStartMs
        return words.map { word ->
            val wordDuration = (totalDuration * word.length) / totalChars
            val syncedWord = SyncedWord(currentTime, word)
